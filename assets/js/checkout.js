document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- HÀM XỬ LÝ ĐẶT HÀNG (TÙY THEO PHƯƠNG THỨC THANH TOÁN) ---
    const handlePlaceOrder = async (button) => {
        ...
        if (orderError) {
            ...
            if (selectedPaymentMethod === 'stripe') {
                // Giả sử bạn có route /payments/stripe?order_id=...
                window.open(`/payments/stripe?order_id=${orderData.id}&amount=${item.price}`, '_blank');
                alert('Vui lòng hoàn tất thanh toán Stripe ở cửa sổ mới. Sau khi thành công sẽ nhận sản phẩm!');
                button.disabled = false;
                button.innerHTML = 'Đặt hàng';
                // KHÔNG chuyển trang ở tab này
            } else if (selectedPaymentMethod === 'paypal') {
                // Route thanh toán paypal
                window.open(`/payments/paypal?order_id=${orderData.id}&amount=${item.price}`, '_blank');
                alert('Vui lòng hoàn tất thanh toán PayPal ở cửa sổ mới. Sau khi thành công sẽ nhận sản phẩm!');
                button.disabled = false;
                button.innerHTML = 'Đặt hàng';
            } else {
                // Ngân hàng hoặc mặc định thì chuyển thankyou luôn
                window.location.href = `/thankyou/?order_id=${orderData.id}`;
            }
        }
    };
    
    // Ngắt sự kiện submit mặc định của form và gán handlePlaceOrder
